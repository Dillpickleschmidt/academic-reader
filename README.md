...
**Development (`bun run dev`):** All modes (local, runpod, datalab) use self-hosted Convex via Docker - no account needed.

**Production (`bun run deploy`):** Requires Convex Cloud. Run `bunx convex dev` in `frontend/` to create a deployment, then add `CONVEX_DEPLOYMENT` and `CONVEX_URL` to `.env.local`. Also add `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` for OAuth.
