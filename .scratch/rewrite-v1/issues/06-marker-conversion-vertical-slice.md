...
Enable Marker as the first Conversion Model. Marker should run locally or on Modal according to deployment config, emit fine-grained Processing Events through a tested tqdm hook, adapt output into Pages and Blocks, and make the Reader View ready from ordered Block HTML.
...
- API now owns Document creation + Marker start via `POST /api/documents`.
...
- Modal Marker model loading now happens inside the per-run method instead of `@modal.enter()`, so cold-start/model-download tqdm output can become Processing Events for the active Document.
