...
Add Convex persistence and Better Auth so a Reader can sign in and view an authenticated empty Source Document library. Keep the Convex folder split between thin `api/` functions and business logic under `model/`.

## Acceptance criteria

- [x] Convex package/app is configured and type generation works.
- [x] Better Auth sign-in/sign-out works in the Solid web app.
- [x] The home route shows an authenticated empty Source Document library.
